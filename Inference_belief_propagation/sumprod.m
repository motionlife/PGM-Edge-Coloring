...
for c = 1:N
    ...
    if num > 1
        ...
        margin = zeros(ones(1,num)*K);
        Msg = squeeze(MsgV2C(V,c,:));
        blf_v = zeros(num,K);
        for i = 1:num
            if (c > V(i))
                blf_v(i,:) = squeeze(blfX(V(i),c,:));
            else
                blf_v(i,:) = squeeze(blfX(c,V(i),:));
            end
        end
        ...
        blfC{c} = blfc;
        %Calculating Bethe Free Energy
        selected = and(blfc ~= 0, margin ~=0);
        cb = blfc(selected);
        mg = margin(selected);
        Hb = Hb + sum(cb.*log(cb./mg));
    end
end
Z = exp(-Hb);
end
